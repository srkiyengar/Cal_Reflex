...
SCAN_RATE = 4           #1 second divided by scan rate is t he joystick scanning
POS_ERROR = 200
...
LOG_FILENAME = 'cal_reflex_movement' + datetime.now().strftime('%Y-%m-%d---%H:%M:%S')
...
class reflex_sf():
    ...
    def __init__(self, usb_channel = '/dev/ttyUSB0', baudrate = 57600):
        ...
        l_limits = [0,13950,16660,15160, 16384]
        u_limits = [0,0,0,0,0]
    ...
    def palm_rest_position(self):   #Returns a list of current lower limit
        rest_limits = [0,0,0,0,0]
        for i in range(1,5,1):
            rest_limits[i] = self.finger[i]["lower_limit"]
        return rest_limits

    def palm_current_position(self): #Returns a list of current position
        ...
        for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if Buttons[11] == 1:
                        ...
                        new_limits = palm.palm_current_position()
